...
class NYUDepthV2Dataset(Dataset):
    ...
    def __init__(
        ...
        root_dir: str,
        split: str = "train",
        transform: Callable[[Image.Image, np.ndarray], tuple[torch.Tensor, torch.Tensor]] | None = None,
        ...
        if len(self.image_paths) == 0:
            raise RuntimeError(f"No images found in {self.root_dir} for split '{self.split}'")
    ...
    def _load_file_paths(self) -> None:
        ...
        if csv_path.exists():
            ...
            df["image"] = df["image"].apply(lambda x: x[5:] if x.startswith("data/") else x)
            df["depth"] = df["depth"].apply(lambda x: x[5:] if x.startswith("data/") else x)
            ...
            for img_path in sorted(img_dir.glob("*_rgb.jpg")):
                depth_path = img_path.parent / img_path.name.replace("_rgb.jpg", "_depth.png")
